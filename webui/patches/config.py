...
v0.2.1 note: _match_provider now accepts an optional ``preset`` parameter.
"""
...
def apply() -> None:
    ...
    def _patched_match_provider(self, model: str | None = None, preset=None) -> tuple:
        ...
        if forced != "auto" and forced in custom_providers:
            ...
            # Return the exact name so the factory can detect it's custom
            return p, forced
        ...
        # 3. Fallback to original nanobot logic (preset is keyword-only)
